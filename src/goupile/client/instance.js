...
function InstanceController() {
            ...
            return html`
                <div class="padded">
                    <div class="ui_quick">
                        ...
                        <div style="flex: 1;"></div>
                        <a @click=${exportToExcel}>Export Excel</a>
                        <div style="flex: 1;"></div>
    ...
    async function exportToExcel() {
        ...
        let range = IDBKeyRange.bound(profile.username + ':processus@', profile.username + ':processus`', false, true);
        let objects = await db.loadAll('rec_records', range);
        ...
        for (let enc of objects) {
            try {
                ...
                if (record.fragments[record.fragments.length - 1].page == null)
                    continue;

                record.values = {};
                for (let i = 0; i < record.fragments.length; i++) {
                    let fragment = record.fragments[i];
                    Object.assign(record.values, fragment.values);
                }
        ...
        const TYPES = {
            1: "RDV honoré",
            2: "RDV non honoré",
            3: "Appel téléphonique émis",
            4: "Appel téléphonique reçu",
            5: "SMS reçu",
            6: "SMS envoyé",
            7: "Mail reçu",
            8: "Mail envoyé",
            9: "Synthèse EMOT",
            10: "Synthèse EMOT + autre structure",
            11: "Accompagnement à la libération"
        };
        const TRANSPORTS = {
            1: "À pied",
            2: "En vélo",
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            XLSX.utils.sheet_add_aoa(ws1, [[
                ...
                values.inclusion,
                values.date_entree,
                values.date_liberation,
                values.date_contact_prev,
                values.date_contact_eff,
                values.date_sortie
            ]], {origin: -1});

            for (let j = 1; j <= 1000; j++) {
                if (values[`type${j}`] == null)
                    break

                let soignants = [];
                for (let k = 1;; k++) {
                    if (values[`soignant${j}_${k}`] == null)
                        break;

                    let txt = values[`soignant${j}_${k}`];
                    txt += '(' + (TRANSPORTS[values[`lieu${j}_${k}`]] || '?') + ')';
        ...
        XLSX.utils.book_append_sheet(wb, ws2, 'Actes');

        // Write file
        let filename = `ProcEMOT_${dates.today()}.xlsx`;
