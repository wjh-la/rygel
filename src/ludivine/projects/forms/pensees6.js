...
function build(form, values) {
    ...
    form.part(() => {
        form.output(html`<i>Au <b>cours du dernier mois</b>, à quel point avez-vous été dérangé par...</i>`)
    ...
    form.part(() => {
        form.binary("p1", "Avoir des rêves perturbants où se rejoue une partie de l’expérience ou qui sont clairement en relation avec l’expérience ?", lik5)
        form.binary("p2", "Avoir des images ou des souvenirs forts (qui viennent à l’esprit) comme si l’expérience se rejoue ici et maintenant ?", lik5)
        form.binary("p3", "Éviter les ressentis qui rappellent l’expérience (par exemple, pensées, sentiments ou sensations physiques) ?", lik5)
    })

    form.part(() => {
        form.binary("p4", "Éviter les éléments extérieurs qui rappellent l’expérience ?", lik5, {
            help: "Par exemple des personnes, lieux, conversations, objets, activités, ou situations"
        })
        form.binary("p5", "Être en état de super-alerte, vigilance ou sur ses gardes ?", lik5)
        form.binary("p6", "Réaction exagérée de surprise ou sursaut ?", lik5)
    })

    form.part(() => {
        form.binary("p7", "Est-ce que cela a affecté vos relations et votre vie sociale ?", lik5)
        form.binary("p8", "Est-ce que cela a affecté votre travail ou votre capacité à travailler ?", lik5)
        form.binary("p9", "Est-ce que cela a affecté d’autres parties importantes de votre vie telles que la capacité à s’occuper de vos enfants, vos études, ou toutes autres activités importantes ?", lik5)
    })
    ...
    form.part(() => {
        form.binary("c1", "Quand je suis contrarié.e, il me faut beaucoup de temps pour me calmer", lik5)
        form.binary("c2", "Je me sens insensible ou émotionnellement éteint(e)", lik5)
        form.binary("c3", "Je me sens nul(le)", lik5)
    })

    form.part(() => {
        form.binary("c4", "Je me sens sans valeur", lik5)
        form.binary("c5", "Je me sens distant(e) ou coupé(e) des autres", lik5)
        form.binary("c6", "je trouve difficile de rester émotionnellement proche des autres", lik5)
    })
    ...
    form.part(() => {
        form.binary("c7", "Créé de l'inquiétude ou de la détresse concernant vos relations ou votre vie sociale", lik5)
        form.binary("c8", "Affecté votre travail ou capacité à travailler", lik5)
        form.binary("c9", "Affecté d'autres parties importantes de votre vie telle que la capacité à s'occuper de vos enfants, vos études, ou toutes autres activités importantes ?", lik5)
    })
